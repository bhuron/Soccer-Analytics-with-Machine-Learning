...
let
	...
	# Helper: compute lagged rolling mean per team
	function team_lag_mean(df, team_col, value_col, n_lag, n_window)
		result = Vector{Union{Missing,Float64}}(missing, nrow(df))
	...
	df_sorted.goals_last_3_avg = team_lag_mean(df_sorted, :team, :goals, 1, 3)
	df_sorted.goals_last_5_avg = team_lag_mean(df_sorted, :team, :goals, 1, 5)
	...
	select(first(ars, 8), :match_week, :goals, :goals_last_3_avg, :recent_form)
end
