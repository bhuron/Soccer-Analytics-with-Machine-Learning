...
let
	...
	match_raw = JSON3.read(read(joinpath(DATA_DIR, "matches", "72", "30.json"), String))
		m_dicts = flatten_dict.(match_raw)
		m_keys = union((keys(d) for d in m_dicts)...)
		m_rows = [let row = Dict{String,Any}()
			for k in m_keys; row[k] = get(d, k, missing); end; row
		end for d in m_dicts]
		matches_df = DataFrame(m_rows)
