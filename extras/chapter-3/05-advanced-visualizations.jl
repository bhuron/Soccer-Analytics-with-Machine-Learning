...
let
	passes = subset(events,
		"type.name" => ByRow(==("Pass")),
		"team.name"  => ByRow(==("France Women's")))
	passes.ex = (endloc -> ismissing(endloc) ? missing : endloc[1]).(passes[!, "pass.end_location"])
	passes.ey = (endloc -> ismissing(endloc) ? missing : endloc[2]).(passes[!, "pass.end_location"])
