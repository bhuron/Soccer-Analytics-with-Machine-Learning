...
md"""
# Analyse d'événements & visualisation

**Chapitre 3 · Analyse exploratoire de données dans le football**

## Ce que vous allez apprendre

- Charger et explorer les données d'événements de StatsBomb
- Filtrer les événements par type, équipe et joueur
- Regrouper et agréger les événements pour créer des résumés pertinents
- Calculer la précision des passes, le taux de conversion des tirs et les buts attendus (xG)
- Créer des visualisations pour révéler les motifs des événements
"""
...
Les données d'événements sont stockées dans un fichier JSON par match. Chaque
fichier contient toutes les actions avec ballon — passes, tirs, tacles, conduites
— qui ont eu lieu pendant ce match.

Commençons par un seul match pour comprendre la structure, puis passons à
...
begin
	...
	md"""**$(nrow(events)) événements** chargés depuis le match 22921 (France vs Corée du Sud) sur $(ncol(events)) colonnes."""
end
...
Chaque événement possède un `type.name` qui décrit l'action.
"""
...
let
	...
	p = plot(xlims=(0, maximum(vals) + 100), ylims=(0.5, n + 0.5),
		legend=false, title="Top 10 des types d'événements du match",
		xlabel="Nombre", yticks=(1:n, labs))
	for i in 1:n
...
md"""
**Ce que cela nous apprend :** les passes dominent (~60–70% des événements),
suivies des réceptions de balle, des conduites et des événements de pressing.
Les tirs et les buts sont rares mais décisifs.
"""

# ╔═╡ d4e5f6a7-0011-5b3c-0e2d-9f0e7d6c5b4a
md"""
## Filtrage — se concentrer sur des événements spécifiques

Examinons les passes de la France : filtrage par type et par équipe.
"""
...
md"""
## Comprendre les résultats des passes

Une passe **réussie** a une valeur *manquante* dans `pass.outcome.name`. Toute
valeur non manquante ("Incomplete", "Out", "Pass Offside") signifie que la
passe a échoué.
"""
...
let
	...
	| Métrique | Valeur |
	|---|---|
	| Total des passes | **$total** |
	| Réussies | **$completed** |
	| Manquées | **$(total - completed)** |
	| Précision des passes | **$(round(100 * accuracy; digits=1))%** |
	"""
end

# ╔═╡ d4e5f6a7-0015-5b3c-0e2d-9f0e7d6c5b4a
md"""
## Chargement multi-matchs

Chargement des événements de 5 matchs, empilés dans un seul grand DataFrame.
"""
...
begin
	...
	md"""**$(nrow(multi_events)) événements** chargés depuis **$(length(match_ids)) matchs** sur $(ncol(multi_events)) colonnes."""
end

# ╔═╡ d4e5f6a7-0017-5b3c-0e2d-9f0e7d6c5b4a
md"""
## Résumé des passes par match

Pour chaque couple (match, équipe) : tentatives de passes, passes réussies, précision.
"""
...
md"""
## Précision des passes par équipe
"""
...
let
	...
	p = plot(xlims=(0.5, 1.0), ylims=(0.5, n + 0.5),
		legend=false, title="Précision moyenne des passes par équipe",
		xlabel="Précision des passes", yticks=(1:n, labs))
	for i in 1:n
...
md"""
## Analyse des tirs et des buts attendus (xG)

Les passes contrôlent le ballon. Les tirs décident du match.
"""
...
let
	...
	xg_msg = if has_xg
		...
		"| xG moyen / tir | **$(round(avg_xg; digits=3))** |\n| xG total | **$(round(total_xg; digits=2))** |"
	else
		"| Données xG | *non disponibles* |"
	end

	md"""
	| Métrique | Valeur |
	|---|---|
	| Total des tirs | **$total_shots** |
	| Buts | **$n_goals** |
	| Taux de conversion | **$(round(100 * conv_rate; digits=1))%** |
	$xg_msg
	"""
end
...
md"""
## Statistiques de tirs par match

Tirs, buts, et (si disponible) xG pour chaque équipe dans chaque match.
"""
...
md"""
## Volume de tirs vs. buts marqués

Plus de tirs → plus de buts ? Vérifions.
"""
...
let
	...
	scatter(x, y, legend=false, markersize=8, color=:darkblue,
		title="Volume de tirs vs Buts marqués",
		xlabel="Tirs par match", ylabel="Buts par match")
	plot!(x_fit, y_fit, color=:blue, linestyle=:dash, linewidth=2, label="Trend")
end

# ╔═╡ d4e5f6a7-0029-5b3c-0e2d-9f0e7d6c5b4a
md"""
**Ce que cela nous apprend :** une corrélation positive claire. Les équipes
au-dessus de la droite de tendance sur-performent (finition clinique) ; les
équipes en dessous doivent améliorer leur conversion.
"""

# ╔═╡ d4e5f6a7-0030-5b3c-0e2d-9f0e7d6c5b4a
md"""
## Distribution de la qualité des tirs (xG)

La plupart des tirs ont une faible probabilité — les occasions de grande
qualité sont rares.
"""
...
let
	...
	if "shot.statsbomb_xg" in names(shots2)
		...
		histogram(xg_vals, bins=25, color=:salmon, alpha=0.8, legend=false,
			title="Distribution de la qualité des tirs (xG)",
			xlabel="xG du tir (buts attendus)", ylabel="Nombre")
	else
...
md"""
**Ce que cela nous apprend :** la plupart des tirs ont un xG < 0,1. Les
occasions au-dessus de 0,3 xG sont rares et précieuses.
"""
...
1. Chargement des données d'événements depuis les fichiers JSON de StatsBomb
2. Filtrage des événements par type, équipe et joueur
3. Regroupement et agrégation pour créer des résumés
4. Calcul de la précision des passes, du taux de conversion des tirs et des totaux xG
5. Création de diagrammes en barres, nuages de points et histogrammes

## Prochaine étape

Visualisations avancées (cartes de tirs, cartes de chaleur), réseaux de passes,
et une étude de cas du Japon à la Coupe du Monde féminine 2019.
"""
...
1. **Longueur de passe** — calculer la longueur moyenne des passes pour chaque
   équipe en utilisant la colonne `pass.length`.
2. **Distance de tir** — analyser la distribution des distances de tir par
   rapport au but.
3. **Classement des joueurs** — trouver les 10 meilleurs joueurs par nombre
   de passes réussies.
4. **Analyse temporelle** — fréquence des tirs par intervalles de 15 minutes.
5. **Actions défensives** — compter les tacles, interceptions et dégagements
   par équipe.
"""
...
md"""
### Exercice 1 — Longueur de passe moyenne par équipe

La colonne `pass.length` donne la distance en mètres.
"""
...
md"""
Passes longues → jeu direct. Passes courtes → style orienté possession.
"""

# ╔═╡ d4e5f6a7-0039-5b3c-0e2d-9f0e7d6c5b4a
md"""
### Exercice 2 — Distance de tir

Distance depuis le centre du but ``(120,40)`` : ``\sqrt{(120-x)^2 + (40-y)^2}``.
"""
...
let
	...
	histogram(clean.distance, bins=20, color=:darkred, alpha=0.8,
		legend=false, title="Distribution des distances de tir",
		xlabel="Distance au but (m)", ylabel="Nombre")
end

# ╔═╡ d4e5f6a7-0041-5b3c-0e2d-9f0e7d6c5b4a
md"""
La plupart des tirs depuis l'intérieur ou juste à l'extérieur de la surface (10–25 m).
"""

# ╔═╡ d4e5f6a7-0042-5b3c-0e2d-9f0e7d6c5b4a
md"""
### Exercice 3 — Top 10 des joueurs par passes réussies
"""
...
md"""
Ce sont les organisateurs du milieu de terrain — les joueurs qui dictent le tempo.
"""

# ╔═╡ d4e5f6a7-0045-5b3c-0e2d-9f0e7d6c5b4a
md"""
### Exercice 4 — Fréquence des tirs par intervalle de 15 minutes
"""
...
let
	...
	p = plot(xlims=(0.5, n + 0.5), ylims=(0, maximum(ys) + 5),
		legend=false, title="Tirs par intervalle de 15 minutes",
		xlabel="Période du match", ylabel="Nombre de tirs",
...
md"""
La fréquence des tirs atteint souvent un pic juste avant la mi-temps et en fin de match.
"""

# ╔═╡ d4e5f6a7-0048-5b3c-0e2d-9f0e7d6c5b4a
md"""
### Exercice 5 — Actions défensives par équipe

Comptage des tacles (Duel), interceptions et dégagements.
"""
...
md"""
Nombre élevé de duels → pressing agressif. Nombre élevé d'interceptions →
